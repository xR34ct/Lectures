...
\subsection{Kurvor \& ytor i \(\mathbb{R}^3\) på parameterform}
...
\(\bar{N} = \frac{\partial \bar{r}}{\partial s} \times \frac{\partial \bar{r}}{\partial t}\)
För planet i ex ovan är \(\frac{\partial \bar{r}}{\partial s} = (1,-1,0) \quad\&\quad \frac{\partial \bar{r}}{\partial t} = (1,0,3)\)
...
\newpage
\subsection{Funktionen \(\mathbb{R}^n \rightarrow \mathbb{R}^m\)}
...
\subsubsection{Exempel 3}
\[
	\bar{f}'(\bar{x}) = \Big(f_1(x_1,x_2),f_3(x_1,x_2)\Big) = (x_1+x_2,x_1x_2) \Rightarrow \frac{\partial(f_1,f_2)}{\partial(x_1,x_2)} = 
	\begin{pmatrix}
		1 & 1 \\
		x_2 & x_1
	\end{pmatrix}
...
\underline{OBS!} För \(m=1 (\bar{f} = f)\) fås specialfallet \(\bar{f}'(\bar{x}) = (f'_{x_1}, \cdots ,f'_{x_n}) = \nabla f\)
\newpage
